...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def descendingByRetweetAcc(acc: TweetList): TweetList = {
   val mR = this.mostRetweeted
    new Cons(this.mostRetweeted, this.remove(this.mostRetweeted).descendingByRetweetAcc(acc))
  }
  ...
  def mostRetweeted: Tweet = {
    if (isLeaf) {
      this.elem
       ...
       max(right.mostRetweeted, this.elem) 
    } else if(right.isEmpty) {
       max(left.mostRetweeted, this.elem)
    } else { 
       max(max(left.mostRetweeted, right.mostRetweeted), this.elem) 
    }
  }
 
  private def max(lt: Tweet, rt: Tweet): Tweet = {
    if (lt.retweets >= rt.retweets) lt
    else rt
  }
