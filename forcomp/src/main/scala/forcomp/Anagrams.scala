...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    ((for {
      (c, s) <- (w groupBy (c => c.toLower))
    } yield (c, s.length())) toList) sortBy(p => p._1) 
  }

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = wordOccurrences((s flatMap (w => w.toList)) mkString "") 
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    ...
    def m1(os : Occurrences) : List[Occurrences] = {
      (for {
        o: (Char, Int) <- os
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    if (y == List())
      ...
      subtract(x.filter(n => n != y.head), y.tail)
    }
  }
